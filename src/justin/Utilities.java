...
public class Utilities {
  ...
  public static boolean fuzzyMove(RobotController rc, Direction dir, int fuzz) throws GameActionException {
    if (rc.canMove(dir)) {
      rc.move(dir);
      return true;
    }
  ...
  public static void fight(RobotController rc) throws GameActionException{
    ...
    if(enemiesWithinAttack.length>0&&!(rc.getActionCooldownTurns()>=10)){
      ...
      fuzzyMove(rc,enemiesWithinAttack[0].getLocation().directionTo(current),2);
    }

    if(rc.getHealth()<attackStrength){
      //retreat cuz low health
      fuzzyMove(rc,enemies[0].getLocation().directionTo(current),2);
    }
    //end retreat logic
  ...
  public static void heal(RobotController rc) throws GameActionException{
    ...
    if(rc.canHeal(weakestAlly.getLocation())){
      ...
      fuzzyMove(rc,current.directionTo(weakestAlly.getLocation()),1);
      if(rc.canHeal(weakestAlly.getLocation())){
        if(rc.canWriteSharedArray(2, rc.readSharedArray(2)+healStrength))
          rc.writeSharedArray(2,rc.readSharedArray(2)+healStrength);
        rc.heal(weakestAlly.getLocation());
